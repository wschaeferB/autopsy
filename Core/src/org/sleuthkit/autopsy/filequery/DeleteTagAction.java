...
public class DeleteTagAction extends AbstractAction {
    ...
    DeleteTagAction(TagName tagName, ContentTag contentTag, long fileId, String notableString) {
    ...
    static public JMenu getTagMenu(AbstractFile file) {
        return new TagMenu(file);
    }

    @NbBundle.Messages({"# {0} - fileID",
        "DeleteTagAction.deleteTag.alert=Unable to untag file {0}."})
        ...
        new SwingWorker<Void, Void>() {
            ...
            protected Void doInBackground() throws Exception {
                try {
                    ...
                    Case.getCurrentCase().getServices().getTagsManager().deleteContentTag(contentTag);
                } catch (TskCoreException tskCoreException) {
                    logger.log(Level.SEVERE, "Error untagging file", tskCoreException); //NON-NLS
                    Platform.runLater(()
                            -> new Alert(Alert.AlertType.ERROR, Bundle.DeleteTagAction_deleteTag_alert(fileId)).show()
                    );
                }
                return null;
            }
                ...
                for (int i = 0; tagNameIterator.hasNext(); i++) {
                    ...
                    for (ContentTag contentTag : existingTagsList) {
                        if (contentTag.getName().getId() == tagName.getId()) {
                            ...
                            DeleteTagAction deleteDrawableTagAction = new DeleteTagAction(tagName, contentTag, file.getId(), notableString);
                            JMenuItem tagNameItem = new JMenuItem(deleteDrawableTagAction);
                            add(tagNameItem);
                            menuEmpty = false;
